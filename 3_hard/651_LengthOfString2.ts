...
type LengthOfString<
  S extends string,
  Res extends string[] = []
> = S extends `${s}${s}${s}${s}${s}${s}${s}${s}${s}${s}${infer Rest}`
  ? LengthOfString<Rest, [...Res, s, s, s, s, s, s, s, s, s, s]>
  : S extends `${s}${infer Rest}`
    ? LengthOfString<Rest, [...Res, s]>
    : Res["length"];

// 根据题意，和 medium 298 不一样的是，需要支持数百个字符长的字符串，如果还是还是一个字符 push 效率太低
